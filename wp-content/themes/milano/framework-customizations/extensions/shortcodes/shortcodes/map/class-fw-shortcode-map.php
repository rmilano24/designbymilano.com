...
class FW_Shortcode_Map extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => esc_html__('Custom','modesto'),
					'options'    => array(
						'locations' => array(
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				esc_html__('No location provider specified for map shortcode', 'modesto')
			);
			return '<b>' . esc_html__( 'Map Placeholder', 'modesto' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
			       . sprintf(esc_html__('Unknown location provider "%s" specified for map shortcode', 'modesto'), $provider)
			       . ' -->';
		}
