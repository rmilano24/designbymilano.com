...
function get_simple_likes_button( $post_id, $is_comment = null ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'modesto' );
		$icon  = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'modesto' );
		$icon  = $icon_empty;
	}
...
function show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'modesto' ); ?></label></th>
			<td>
				<?php
				...
				<?php else : ?>
					<p><?php _e( 'You do not like anything yet.', 'modesto' ); ?></p>
					<?php
				endif;
				wp_reset_postdata();
				?>
